...
class ScreenAwakeStatusProvider extends ChangeNotifier {
  ...
  Future<void> updateScreenAwakeStatus({bool? value}) async {
    _isScreenAwake = value ?? await dam.isScreenAwake();
    notifyListeners();
  }
}
...
Future<void> enableKioskMode() async {
  try {
    await dam.setAsLauncher();
    await dam.lockApp();
    await dam.setKeepScreenAwake(true);
    _statusProvider.updateScreenAwakeStatus(value: true);

    await dam.put(bootCompletedHandlerStartedKey, content: "false");
  } catch (e) {
    debugPrint('dam:: enableKioskMode error: $e');
  }
}

Future<void> initKioskMode() async {
  dam.handleBootCompleted((_) async {
    final startedValue = await dam.get(bootCompletedHandlerStartedKey);
    final isStarted = startedValue == "true";
    debugPrint('dam:: handleBootCompleted:: isStarted: $isStarted');
    await dam.put(bootCompletedHandlerStartedKey, content: "true");
    if (!isStarted) {
      try {
        await dam.startApp();
      } catch (e) {
        debugPrint('dam:: handleBootCompleted startApp error: $e');
      }
    }
  ...
  final startedValue = await dam.get(bootCompletedHandlerStartedKey);
  final isStarted = startedValue == "true";
  debugPrint('dam:: init:: startedValue $startedValue, isStarted: $isStarted');
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          TextButton(
            onPressed: () {
              dam.setKeepScreenAwake(!_statusProvider.isScreenAwake).then(
                (value) {
