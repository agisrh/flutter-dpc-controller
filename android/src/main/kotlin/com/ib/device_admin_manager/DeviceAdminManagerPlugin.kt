...
class DeviceAdminManagerPlugin : FlutterPlugin, MethodCallHandler, ActivityAware,
    ...
    private fun setKeyguardDisabled(disabled: Boolean, result: Result) {
        if (!isAdminActive()) {
            return result.error("ADMIN_NOT_ACTIVE", "Admin privileges must be activated first.", null)
        }
    ...
    private fun clearDeviceOwnerApp(packageName: String?, result: Result) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            try {
                ...
                result.error("NOT_DEVICE_OWNER", "This function can only be called if the app is a Device Owner.", e.localizedMessage)
            } catch (e: Exception) {
                result.error("ERROR", e.localizedMessage, null)
            }
        } else {
            result.error("UNSUPPORTED_VERSION", "This feature is not supported on this Android version.", null)
        }
    }
    ...
    private fun wipeData(flags: Int, reason: String?, result: Result) {
        ...
        try {
            ...
            result.error("SET_WIPE_DATA", e.localizedMessage, null)
        } catch (e: Exception) {
            result.error("ERROR", e.localizedMessage, null)
        }
    }
